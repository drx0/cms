...
function DB_Select($link, $query, $data){
	// data
...
// $result = DB_Select($db_link, 'SELECT * FROM `cms_backed_users` WHERE login = :login', ['login'=>$cookie]);

function DB_Insert($link, $table_name, $paste){
	$keys = '(';
	$values = '(';
...
// $result = DB_Insert('frontendusers', [
...
function DB_Update($link, $table_name, $updates, $wheres){
	// updates
	$values = '';
